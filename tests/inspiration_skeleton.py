...
__author__ = "Dominik Pataky"
__copyright__ = "Dominik Pataky"
__license__ = "mit"
...
class TestSoftflowExport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # FIXME: templates are saved between test runs, because they are
        # stored with the class. Maybe the templates should be stored
        # with an instance?
        logging.debug("Resetting SoftflowUDPHandler templates.")
        SoftflowUDPHandler.templates = {}

        logging.debug(
            "Setting temporary file {} as output for SoftflowUDPHandler".format(
        ...
        logging.debug(
            "Creating and running the Softflow collector in another thread."
        )
    ...
    def _test_export(self):
        ...
        template_idx = random.randint(
            1, self.COUNT_PACKETS_TO_TEST - 1
        )  # 1 for enhanced testing, -1 because randint
        ...
        for idx in range(self.COUNT_PACKETS_TO_TEST):
            ...
            if idx == template_idx:
                ...
                self.sock.sendto(
                    bytes.fromhex(template_packet), self.CONNECTION
                )
                lens.append(8)
                sleep(self.SLEEP_TIME)
        ...
        # We got four exports
        logging.info(
            "Testing the existence of all exports, including the ones with formerly unknown templates: {} of {}".format(
                len(exported.keys()), self.COUNT_PACKETS_TO_TEST + 1
            )
        )
        self.assertEqual(
            len(exported.keys()), self.COUNT_PACKETS_TO_TEST + 1
        )  # +1 including the template packet
        ...
        if self.RUN_ANALYZER:
            ...
            analyzer = subprocess.run(
                ["python3", "analyze_json.py", self.temp_output_file.name],
